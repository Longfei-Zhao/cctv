    ...
    drawImage: function(imagePath)
    {
      ...
      var img = new Image();
        ...
        $(img).load(function(){
          ...
          $this.ctx.drawImage(img, 0, 0, __w, __h, 0, 0, $this.canvas.width, $this.canvas.height);
        //console.log($this.canvas.width,$this.canvas.height, $(window).width(), $(window).height());
          $this.setBgImage();
        })
    },
...
function init_wScratchPad(box, image2, conf){
  conf = conf || {};
  var sp = box.wScratchPad({
    scratchDown: function(){
      $(this.canvas).css('margin-right', $(this.canvas).css('margin-right') == "0px" ? "1px" : "0px");
      if(conf.down) conf.down.apply(this, arguments);
    },
    scratchMove: function(){
      $(this.canvas).css('margin-right', $(this.canvas).css('margin-right') == "0px" ? "1px" : "0px");
      if(conf.move) conf.move.apply(this, arguments);
    },
    scratchUp: function(e, percent){
      $(this.canvas).css('margin-right', $(this.canvas).css('margin-right') == "0px" ? "1px" : "0px");
      if(conf.up) conf.up.apply(this, arguments);
    },
    width           : $(window).width(),                  // set width - best to match image width
    height          : $(window).height(),                  // set height - best to match image height
    image           : '',  // set image path
    image2          : image2,                 // set overlay image path - if set color is not used
    //color           : '#336699',            // set scratch color - if image2 is not set uses color
    overlay         : 'none',               // set the type of overlay effect 'none', 'lighter' - only used with color
    size            : 100,                   // set size of scratcher
    realtimePercent : false,                // Update scratch percent only on the mouseup/touchend (for better performances on mobile device)
